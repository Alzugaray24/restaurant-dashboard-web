...
export function CustomersList() {
  ...
  // Añadir estado para mantener las fechas formateadas y conteos de órdenes
  const [customerDetails, setCustomerDetails] = useState<Record<number, { formattedDate: string, ordersCount: number }>>({});
  
  // Mover la lógica de formateo de fechas a un useEffect para que se ejecute solo en el cliente
  useEffect(() => {
    const details: Record<number, { formattedDate: string, ordersCount: number }> = {};
    ...
    customers.forEach(customer => {
      ...
      details[customer.id] = { formattedDate, ordersCount };
    });
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {currentCustomers.length > 0 ? (
              currentCustomers.map((customer) => {
                // Usar los detalles del cliente del estado
                const details = customerDetails[customer.id] || { formattedDate: 'N/A', ordersCount: 0 };
